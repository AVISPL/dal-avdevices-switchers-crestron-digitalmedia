...
public class DMFrame extends TelnetCommunicator implements Pingable, Monitorable, Controller {
    ...
    private int INPUT_SLOTS = 16;
    private String ctrlCombineMode = "0";
    ...
    public List<Statistics> getMultipleStatistics() throws Exception {
        ...
        final String prompt = send(""); //Send newline in case there is another client connected (a command is preloaded and anything send is appended to the end)
        if (regexFind(prompt,"DM-MD(\\d+)x\\d+>").equals("32")){
            INPUT_SLOTS = 32;
        } else {
            INPUT_SLOTS = 16;
        }
        ...
        AdvancedControllableProperty.Preset combineModePreset = new AdvancedControllableProperty.Preset();
        combineModePreset.setOptions(new String[]{"0","1","2"});
        combineModePreset.setLabels(new String[]{"Separate Control","A/V Combine","A/V/U Combine"});
        ctrls.add(new AdvancedControllableProperty("combineRoutes",new Date(),combineModePreset,ctrlCombineMode));
        stats.put("combineRoutes",ctrlCombineMode);
        ...
        for (String slot : slots){
            if (slot.contains("Card at Slot")){
                ...
                if (!(slotNumber = regexFind(lines[0],"Input Card at [sS]lot (\\d+)")).isEmpty()){
                    ...
                    slotNumber = String.valueOf((Integer.parseInt(slotNumber) - INPUT_SLOTS));
                    for (String line : lines){
                        ...
                        if (!(type = regexFind(line,"(Video|Audio|USB)")).isEmpty()){
                            if (inputSlot.isEmpty()){ //Route is blank
                                ...
                                ctrls.add(createRouteDropdown("Output" + slotNumber + type, inputs,"0"));
                            } else {
                                stats.put("Output" + slotNumber + type, inputSlot);
                                ctrls.add(createRouteDropdown("Output" + slotNumber + type, inputs,inputSlot));
                            }
                        }
                    }
                }
    ...
    private AdvancedControllableProperty createRouteDropdown(String name, List<String> inputs, String selectedInput) {
    ...
    public void controlProperty(ControllableProperty cp) throws Exception {
        ...
        if (cp.getProperty().startsWith("Output")) {
            cp.setProperty(cp.getProperty().substring(6));
            final String input = String.valueOf(cp.getValue());
            final int output = Integer.parseInt(regexFind(cp.getProperty(), "^\\d+"));
            String controlType = cp.getProperty().replace("\\d+","");
            if (controlType.equals("Video") || controlType.equals("Audio") || controlType.equals("USB")) {
                switch (ctrlCombineMode) {
                    case "0":
                ...
                send("SET" + controlType + "ROUTE " + input + " " + (output + INPUT_SLOTS));
            } else {
                if (logger.isDebugEnabled()){
                    logger.debug("Controllable property type " + controlType + " does not exist.");
                }
            }
        } else if (cp.getProperty().equals("combineRoutes")){
            ctrlCombineMode = cp.getProperty();
        }
    }
